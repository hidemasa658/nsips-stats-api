...
def dashboard(
    ...
    # 8 カテゴリを 加算コード (令和6年&8年両対応) で正確に matching → 月別 SUM(count)
    CHIIKI_CATEGORIES = [
        ("夜間・休日等の対応実績", "#dc2626", [
            "450000870",  # 夜間・休日等加算
            "430000670", "440014170", "450000470",  # 時間外加算 (調製料/管理料/基本料)
            "430000770", "440014370", "450000570",  # 休日加算
            "430000970", "440014470", "450000770",  # 時間外加算の特例
        ]),
        ("麻薬の調剤実績", "#7c3aed", [
            "440000970",  # 麻薬管理指導加算 (在宅)
            "440012770",  # 在宅患者医療用麻薬持続注射療法加算
            "440017170", "440017270", "440017370",  # 麻薬関連 (旧)
        ]),
        ("残薬調整・薬学的有害事象防止", "#059669", [
            "440023570", "440023670", "440023770", "440023870",  # 調剤時残薬調整加算 (令和8)
            "440024370", "440024470", "440024570", "440024670",  # 薬学的有害事象等防止加算 (令和8)
            "440011470", "440011570",  # 重複投薬・相互作用等防止加算 (令和6)
            "440005170",  # 旧
        ]),
        ("かかりつけ薬剤師", "#0891b2", [
            "440025110",  # 令和8: 服薬管理指導料1(イ)(かかりつけ)
            "440025210",  # 令和8: 服薬管理指導料2(イ)(かかりつけ)
            "440012210",  # 令和6: 服薬管理指導料2(イ)(かかりつけ)
            "440025510",  # 令和8: 特例1(イ)
            "440025610", "440012910",  # 令和8/6: 特例2(イ)
            "440026370", "440026470",  # かかりつけ薬剤師フォローアップ/訪問加算
            "440004010",  # 旧かかりつけ薬剤師指導料
        ]),
        ("外来服薬支援料1", "#ea580c", [
            "440013510",  # 外来服薬支援料1のみ (2は集計不要)
        ]),
        ("在宅薬剤管理 (単一建物1人)", "#65a30d", [
            "440005710",  # 在宅患者訪問薬剤管理指導料 (単一建物1人)
        ]),
        ("服薬情報等提供料", "#c026d3", [
            "440009970", "440010070", "440010170",  # 服薬情報等提供料 1/2/3
        ]),
        ("小児特定加算", "#f59e0b", [
            "440012370",  # 小児特定加算 (服薬管理指導料)
            "440012970",  # 小児特定加算 (在宅患者訪問)
        ]),
    ]
    ...
    def _chiiki_data(codes: list) -> list:
        """指定コード群の加算の月別合計 count を [(ym, n), ...] で返す。直近12ヶ月。"""
        ...
        rows = conn.execute(
            f"""SELECT SUBSTR(p.dispense_date, 1, 6) AS ym, SUM(COALESCE(f.count, 1)) AS n
                ...
                WHERE p.dispense_date IS NOT NULL
                  ...
                  AND f.code IN ({placeholders})
                GROUP BY ym ORDER BY ym""",
            [_chiiki_min_ym] + codes,
        ).fetchall()
